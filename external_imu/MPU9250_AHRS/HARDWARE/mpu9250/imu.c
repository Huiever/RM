...
uint8_t MPU6500_Init(void)
{
  ...
  if(MPU_id == MPU6500_Read_Reg(MPU6500_WHO_AM_I))  //read id of device,check if MPU6500 or not
  {
...
//读取磁力计的数据
void GetIST8310_RawValues()
{
	uint8_t mpu_buff[6];
	MPU6500_Read_Regs(MPU6500_EXT_SENS_DATA_00,mpu_buff,6);
	imu_data.mx = mpu_buff[1]<<8 |mpu_buff[0] - imu_data_offest.mx;
	imu_data.my = mpu_buff[3]<<8 |mpu_buff[2] - imu_data_offest.my;
	imu_data.mz = mpu_buff[5]<<8 |mpu_buff[4] - imu_data_offest.mz;
}
