...
void IMU_getValues(volatile float * values) {  
  ...
  MPU6050_getMotion6(&accgyroval[0], &accgyroval[1], &accgyroval[2], &accgyroval[3], &accgyroval[4], &accgyroval[5]);
	IMU_RAW_Data.ax = accgyroval[0];
	IMU_RAW_Data.ay = accgyroval[1];
	IMU_RAW_Data.az = accgyroval[2];
	IMU_RAW_Data.gx = accgyroval[3];
	IMU_RAW_Data.gy = accgyroval[4];
	IMU_RAW_Data.gz = accgyroval[5];
	...
	AK8963_mgetValues(&values[6]);	//读取磁力计的ADC值
	IMU_RAW_Data.mx = values[6];
	IMU_RAW_Data.my = values[7];
	IMU_RAW_Data.mz = values[8];
}
/**************************实现函数********************************************
...
{
	IMU_Real_Data.gy = -mygetqval[4];
	IMU_Real_Data.gz = mygetqval[5];
